...
<b>Email: <a href="">{{ $setting->email}}</a></b>
        ...
        <div class="col-12">
            <div>
                <div class="p-2">
                    <h3 class="text-center"><u><strong>TERMLY FINANCIAL STATEMENT REPORT FOR TERM : {{$school_details->term}} | YEAR : {{$school_details->year}}</u>  &nbsp;
...
@php
  ...
  $total_amount_collected = App\Models\student_fees::where('term',$value->term)->where('class',$value->class)->where('year',$value->year)->sum('fee_amount');
...
<div class="row" >
  ...
  $price_paid = App\Models\purchase_stocks ::where('term',$item->term)->where('purchase_id',$item->purchase_id)->where('year',$item->year)->sum('total_price');
